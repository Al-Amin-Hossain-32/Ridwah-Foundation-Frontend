...
export default function ReactionButton({
  ...
  const handleQuickClick = useCallback(() => {
    ...
    dispatch(toggleReaction({ targetType, targetId, reactionType: type, postId, userId: currentUser?._id }));
  }, [userReaction, showPicker, targetType, targetId, postId, dispatch, currentUser]);

  const handlePickReaction = useCallback((type) => {
    setShowPicker(false);
    dispatch(toggleReaction({ targetType, targetId, reactionType: type, postId, userId: currentUser?._id }));
 ...
 */
export function ReactionSummary({ counts = {}, total = 0, targetType, targetId }) {
  ...
  const handleMouseEnter = async () => {
    ...
    hoverTimer.current = setTimeout(async () => {
      ...
      if (!tooltip) {
        ...
        try {
          const res = await reactionService.getReactors(targetType, targetId, null, 20);
  ...
  const displayReactors = tooltip?.reactors?.filter(
            ...
            {tooltip?.loading ? (
              ...
              displayReactors.map((reactor, i) => (
                <Link
                          ...
                          >
                <div key={i} className="flex items-center gap-2.5 px-3 py-1.5 hover:bg-gray-50">
                  {/* Avatar */}
